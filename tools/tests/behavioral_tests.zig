...
test "parent navigation uses in-node parent indexes" {
    ...
    defer doc.deinit();
    const child_before = firstQuery(doc.query("span#child")) orelse return error.TestUnexpectedResult;
    try std.testing.expectEqual(@as(u32, 1), child_before.raw().parent);

    const child = firstQuery(doc.query("span#child")) orelse return error.TestUnexpectedResult;
...
test "queries that need ancestry work with in-node parent pointers" {
    ...
    var doc = try default_options.parse(std.testing.allocator, &input);
    defer doc.deinit();
    try std.testing.expectEqual(@as(u32, 2), (firstQuery(doc.query("#c")) orelse return error.TestUnexpectedResult).raw().parent);

    try std.testing.expect(firstQuery(doc.query("#a #c")) != null);
}
...
test "attr-only queries leave parent pointers unchanged" {
    ...
    var doc = try default_options.parse(std.testing.allocator, &input);
    defer doc.deinit();
    const node = firstQuery(doc.query("#a")) orelse return error.TestUnexpectedResult;
    try std.testing.expectEqual(@as(u32, 0), node.raw().parent);

    try std.testing.expect(firstQuery(doc.query("div#a[class=x]")) != null);
    try std.testing.expectEqual(@as(u32, 0), node.raw().parent);
}

test "query yields matches in document preorder" {
    const input =
        "<div id='a'>" ++
...
test "element navigation skips text nodes for sibling/child helpers" {
    ...
    const root = firstQuery(doc.query("div#r")) orelse return error.TestUnexpectedResult;
    var root_children = root.children();
    const first = root_children.next() orelse return error.TestUnexpectedResult;
...
test "parser remains permissive on malformed nesting" {
    ...
    try std.testing.expect(firstQuery(doc.query("#a")) != null);
    try std.testing.expect(firstQuery(doc.query("#b")) != null);
